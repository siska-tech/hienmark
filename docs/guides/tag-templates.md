...
このガイドでは、タグテンプレート機能の基本的な使い方から詳細な仕様まで説明します。
...
3. **タグを追加:**
   - キー: `status`, 値: `pending`, 型: `文字列` → 「追加」
   - キー: `priority`, 値: `high`, 型: `文字列` → 「追加」
   - キー: `type`, 値: `bug`, 型: `文字列` → 「追加」
   - キー: `labels`, 値: `bug, needs-investigation`, 型: `配列（カンマ区切り）` → 「追加」
...
- キー: `status`, 値: `draft`, 型: 文字列
- キー: `priority`, 値: `low`, 型: 文字列
- キー: `type`, 値: `docs`, 型: 文字列
- キー: `labels`, 値: `documentation`, 型: 配列
