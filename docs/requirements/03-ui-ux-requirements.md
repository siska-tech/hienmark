...
#### REQ-UIX-001: フォルダ機能 ⏳ 未実装

**要件概要:**
REQ-ARC-001 (SQLite) のデータベース構造上で、タスクリストを階層的（フォルダ）に分類・管理できる仮想UIを実装する。

**機能要件:**
- FR-2.3.1.1 (UI): ファイルエクスプローラ風のツリービューUIを実装する
- FR-2.3.1.2 (CRUD): フォルダの作成、名称変更、削除、ネスト（階層化）操作を可能にする
- FR-2.3.1.3 (Drag & Drop): タスクをフォルダ間、またはフォルダの階層間でドラッグ＆ドロップにより移動できる

**受入基準:**
- AC-2.3.1.1: ユーザーがタスクを論理的なグループ（フォルダ）に分類し、管理できる
...
| UI/UX要件 | 完成度 | 備考 |
|----------|--------|------|
| R-3.1: カラーテーマ | 100% | ダーク/ライト完全対応 |
| R-3.2: レイアウトカスタマイズ | 100% | ドラッグリサイズ対応 |
| R-3.3: レスポンシブデザイン | 100% | 全画面サイズ対応 |
| R-3.4: ワークスペース管理 | 100% | 自動読み込み対応 |
| R-3.5: 自動保存 | 100% | 3秒デバウンス |
| R-3.6: ファイル監視 | 100% | リアルタイム同期 |
| R-3.7: Front Matter管理 | 100% | GUI単一真実源 |
| R-3.8: ブランディング | 100% | カスタムアイコン |
| R-3.9: 国際化 | 100% | 3言語完全対応 |
| R-3.10: フォルダ機能 | 0% | 未実装（優先度: 高） |
