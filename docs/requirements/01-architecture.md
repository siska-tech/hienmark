...
### REQ-ARC-001: タスクリストのSQLite化 ⏳ 未実装

**要件概要:**
タスクおよび関連エンティティ（タグ、フォルダ、アセット）のプライマリ永続化層を、現在のファイルベース管理から、ワークスペース単位の単一SQLiteデータベースファイル（`.hienmark/tasks.db`）に移行する。

**目的:**
- 高速な検索・ソート機能の実装
- 10,000タスク以上のスケーラビリティ確保
- データ整合性の強化

**機能要件:**
- FR-2.1.1.1: タスク、フォルダ、タグ、アセットを管理する正規化されたリレーショナルスキーマ定義
- FR-2.1.1.2: CRUD操作を行う抽象化されたデータアクセスレイヤー（リポジトリパターン）実装
- FR-2.1.1.3: 既存Markdownファイルベースプロジェクトの自動マイグレーション機能

**非機能要件:**
- NFR-2.1.1.1: 10,000タスクでの検索・ソート操作が500ms未満
- NFR-2.1.1.2: トランザクションによるデータ整合性保証

**受入基準:**
- AC-2.1.1.1: 既存プロジェクトを開いた際、自動的にSQLite DBに移行され正常動作する
- AC-2.1.1.2: 10,000件のダミータスクでパフォーマンステストをクリアする

**実装状況:** ⏳ 未実装（優先度: 高）

---

### REQ-ARC-002: 分析画面のECharts作図への完全移行とMarkdown上でのMermaid記法サポートの維持 ✅ 実装完了

**要件概要:**
安定性確保のため、「分析画面（専用ビュー）」からMermaidベースの作図機能を削除する。ただし、Markdownエディタのビジュアルビュー（プレビュー）におけるMermaid記法のレンダリング機能は維持する。

**背景:**
- 現在の分析画面で、Mermaid作図によりレイアウトが崩れる事象が発生している
- Mermaid記法自体は有用なため、プレビュー機能として維持

**機能要件:**
- FR-2.1.2.1: 分析画面UI上から「ECharts / Mermaid」切替ボタンを削除 ✅
- FR-2.1.2.2: Markdownエディタでのmermaidコードブロックレンダリング機能を保証 ✅

**受入基準:**
- AC-2.1.2.1: 「分析画面」からECharts/Mermaid切替ボタンが表示されない ✅
- AC-2.1.2.2: Markdownエディタで `` ```mermaid `` ブロックが正しくグラフとして表示される ✅

**実装状況:** ✅ 実装完了
- `AnalysisDashboard.tsx` から Mermaid/ソース切替ボタンを削除
- ECharts表示に一本化
- `TaskEditor.tsx` の Mermaidプレビュー機能は維持
